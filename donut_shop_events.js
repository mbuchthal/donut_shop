...
var handleNoteSubmit = function(event) {
  ...
  if (!(event.target.storelocation.value || event.target.mincustomers.value || event.target.maxcustomers.value || event.target.averagedonuts.value)) {             // this is to prevent a blank note from being made
      return alert('note cannot be blank');
    }
  var newStore = new Store(event.target.storelocation.value, event.target.mincustomers.value, event.target.maxcustomers.value, event.target.averagedonuts.value);            // this is to prevent a blank note from being made
